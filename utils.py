...
def load_descriptors(filepath, buffer_size = 256):
    """
    Renvoie une liste de tuples, chacun de dimension 128, représentant un descripteur local
...
def load_database_descriptors(indir, verbose = False):
    files_paths = [indir+'/'+f for f in os.listdir(indir) if os.path.isfile(indir+'/'+f)]
