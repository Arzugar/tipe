...
def compute_descriptor_img(infile, outfile = '', nfeatures = DEFAULT_N_FEATURES, safe_path = False) :
    if not safe_path :
    ...
    # format de sortie : n : nbr de descripteur : 4 bytes, suivit de 128 * n flottants, chacun sur 4 bytes
        with open(outfile, "wb") as outfile :
            ...
            for d in des :
                for f in d :
                    outfile.write( struct.pack("<f", f)) # c'est très probablement des fottants 32 bits donc f est ok 
    else :
        return des

def compute_descriptor_dir(indir, outdir, nfeatures = DEFAULT_N_FEATURES, safe_path = False, verbose = False):
    if not safe_path : 
        indir = os.path.abspath(indir)
        outdir = os.path.abspath(outdir)
    files = [indir+'/'+f for f in os.listdir(indir) if os.path.isfile(indir+'/'+f)]
    if verbose :
        it = tqdm(files)
    else :
        it = files
    for f in it :
        try :
            ...
            compute_descriptor_img(f, outfile=outdir+'/'+'_descr'+filename, nfeatures=nfeatures, safe_path= True)
        except Exception :
